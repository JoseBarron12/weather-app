...
function getSkyGradient(time, sunrise = 6, sunset = 18, isClear = true) {
  ...
  const gradients = {
    night: isClear ? ["#0B132B", "#1C2541"] : ["#1F2937", "#374151"],
    dawn: isClear ? ["#D2691E", "#FFD580"] : ["#4A4A4A", "#A0A0A0"],
    day: isClear ? ["#0077BE", "#B0E0E6"] : ["#64748B", "#A8A8A8"],
    sunset: isClear ? ["#8B0000", "#FFB347"] : ["#5A5A5A", "#A9A9A9"],
  };
  ...
  if (hour < sunrise - 0.5) {
    ...
    [topColor, bottomColor] = blendGradients(
      gradients.night,
      gradients.dawn,
      t,
    );
...
export const display = (function () {
  const header = (weatherDataObj) => {
    ...
    locationName.textContent = upperCaseSentence(
      weatherDataObj.getLocationName(),
    );
  ...
  const twentyFourHourForecast = (desc, arrayOfHours, weatherDataObj) => {
    ...
    if (compareAsc(currentTime, sunrise) == 1) {
      // IF TIME IS PAST SUNRISE
    ...
    for (let i = 0; i < arrayOfHours.length; i++) {
      ...
      if (
        getHours(sunrise) === getHours(arrayOfHours[i].date) &&
        !displaySunrise
      ) {
        const forecastDiv = document.createElement("div");
        forecastDiv.classList.add("forecast-hour");
      ...
      if (
        getHours(sunset) === getHours(arrayOfHours[i].date) &&
        !displayedSunset
      ) {
        const forecastDiv = document.createElement("div");
        forecastDiv.classList.add("forecast-hour");
  ...
  const feelsLike = (weatherDataObj) => {
    ...
    if (weatherDataObj.getFeelsLikeTemp() < weatherDataObj.getCurrentTemp()) {
      ...
      weatherDataObj.getFeelsLikeTemp() > weatherDataObj.getCurrentTemp()
    ) {
  ...
  const wind = (weatherDataObj) => {
    ...
    windDir.textContent =
      weatherDataObj.getWindDirection() +
      "\u00B0 " +
      getWindDirection(weatherDataObj.getWindDirection());
  };
  ...
  const sun = (weatherDataObj) => {
    ...
    if (compareAsc(currentTime, sunset) == 1) {
      // time is past sunset
  ...
  const precipitation = (weatherDataObj) => {
    ...
    precipitationValue.textContent =
      weatherDataObj.getAmountOfPrecipitation() + '"';

    const precipitationDesc = document.querySelector(".precipitation-warning");
    precipitationDesc.textContent =
      "The chance of precipitation is " +
      weatherDataObj.getChanceOfPrecipitation() +
      "%.";
  };
  ...
  const visibility = (weatherDataObj) => {
    ...
    if (visibilityData < 4) {
      ...
      visibilityDesc.textContent =
        "Reduced Visibility — Objects are visible but not far.";
  ...
  const average = (weatherDataObj) => {
    ...
    const differenceText =
      weatherDataObj.getMaxTemp() - weatherDataObj.averageHighTemp;
  ...
  const fullPage = (weather) => {
    ...
    const sunriseHr =
      weather.getSunrise().getHours() + weather.getSunrise().getMinutes() / 60;
    const sunsetHr =
      weather.getSunset().getHours() + weather.getSunset().getMinutes() / 60;

    body.style.background = getSkyGradient(
    ...
    display.twentyFourHourForecast(
  ...
  const initialPage = () => {
    ...
    const editListBtn = document.querySelector(
      ".drop-down-option:first-of-type",
    );
    ...
    display.circleSection(
      circleSection,
      weatherClassData.length,
      currentWeatherPage.currentPage,
    );
    ...
    leftIcon.addEventListener("click", () => {
      circles[currentWeatherPage.currentPage].classList.toggle(
        "current-circle",
      );
  ...
  const location = (weatherDataObj, parent, window, index) => {
    ...
    const sunriseHr =
      weatherDataObj.getSunrise().getHours() +
      weatherDataObj.getSunrise().getMinutes() / 60;
    const sunsetHr =
      weatherDataObj.getSunset().getHours() +
      weatherDataObj.getSunset().getMinutes() / 60;

    locationDiv.style.background = getSkyGradient(
      weatherDataObj.getCurrentDateTime(),
      sunriseHr,
      sunsetHr,
      true,
    );
  ...
  const searchResult = (parent, weatherDataObj, time) => {
    ...
    searchDiv.textContent = toUpperCaseFirstChar(
      weatherDataObj.resolvedAddress,
    );
