...
export const callAPI = (function () {
  const searchResult = async (place, date) => {
    try {
      const url =
        ...
        place +
        "/" +
        getFormattedTdyDate(date) +
        "/?key=49ZTX4PRUA3RHVYAB4P3965HV" +
        "&include=resolvedAddress,current";
  ...
  const moon = async (place, date) => {
    try {
      ...
      const response = await fetch(
        url + "&elements=moonphase,moonriseEpoch,moonsetEpoch",
      );
      const weatherData = await response.json();
      return weatherData;
    } catch (error) {
  ...
  const allWeatherData = async (place, date) => {
    const results = await Promise.all([
      callAPI.weather(place, date),
      callAPI.averageWeather(place, date),
      callAPI.moon(place, date),
    ]);
